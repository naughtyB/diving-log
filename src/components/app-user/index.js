...
import AppUserLogin from './app-user-login/index.js';
import AppUserRegister from './app-user-register/index.js';
import AppUserResetPassword from './app-user-modifyPassword/index.js';
  ...
  render(){
    ...
    return (
      <div className="app-user" style={{position: 'relative'}}>
        <Button type="primary" onClick={()=>this.props.history.push({pathname: '/'})} style={{position: 'absolute', top: '30px', left: '30px'}}>回到首页</Button>
        <div className="top">
          ...
          <span className="top-content">Let's Diving</span>
        </div>
        <div className="desc">
        Let's Diving 专注于潜水日志
        </div>
        {
          pathname === '/login' ? <AppUserLogin history={this.props.history}/> : (pathname === '/register' ? <AppUserRegister history={this.props.history}/> : <AppUserResetPassword history={this.props.history}/>)
        }
      </div>
    )
  }
...
const mapDispatchToProps = (dispatch) => {
  return {
    onChangeUserLoginState: (loginState, userData) => dispatch(doChangeUserLoginState(loginState, userData))
  }
}
